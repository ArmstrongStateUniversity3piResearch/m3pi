...
void do_move()
{
	unsigned char dir = read_next_byte();
	if(check_data_byte(dir))
		return;

	unsigned char units = read_next_byte();
	if(check_data_byte(units))
		return;
...
void setFollowSegmentSpeed()
{
	unsigned char speed = read_next_byte();
	
	if(check_data_byte(speed))
		return;
		
	followSegmentMaxSpeed = speed;
}
